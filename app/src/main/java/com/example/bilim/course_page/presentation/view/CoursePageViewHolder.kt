...
class CoursePageViewHolder(itemView: View) : BaseViewHolder<CourseNameListModel>(itemView) {
   ...
   private var courseIcon: ImageView = itemView.findViewById(R.id.course_item_number_text_view)
    private var courseBackground:ConstraintLayout = itemView.findViewById(R.id.course_item_constraint)
    ...
    override fun onBind(model: CourseNameListModel) {
        ...
        Glide.with(courseIcon.context)
                .load(model.iconUrl)
                .into(courseIcon)
        courseBackground(model)
    }
    @SuppressLint("Range")
    private fun courseBackground(model:CourseNameListModel){
        courseBackground.setBackgroundColor(Color.parseColor(model.bgColor))
    }
}